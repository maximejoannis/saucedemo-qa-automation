...
test.describe("US04 - Checkout | Informations client", () => {
  ...
  test("TC-US04-AC02-02 nom avec tiret accepté", async ({
    page,
    checkoutPage,
  }) => {
    await checkoutPage.fillCustomer({
  ...
  test("TC-US04-AC02-03 code postal alphanumérique accepté", async ({
    page,
    checkoutPage,
  }) => {
    await checkoutPage.fillCustomer({
  ...
  test("TC-US04-AC02-05 annulation retourne au panier", async ({
    page,
    checkoutPage,
  }) => {
    await expect(page).toHaveURL(/\/checkout-step-one\.html$/);
