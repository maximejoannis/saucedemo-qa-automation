...
test.describe('US04 - Checkout | Informations client', () => {
  test.beforeEach(
    async ({ authenticatedPage: _authenticatedPage, inventoryPage, cartPage, page }) => {
  ...
  test('TC-US04-AC02-02 nom avec tiret accepté', async ({ page, checkoutPage }) => {
    await checkoutPage.fillCustomer({
  ...
  test('TC-US04-AC02-03 code postal alphanumérique accepté', async ({ page, checkoutPage }) => {
    await checkoutPage.fillCustomer({
      firstName: validCustomer.firstName,
      lastName: validCustomer.lastName,
      postalCode: 'H2X 1Y4',
