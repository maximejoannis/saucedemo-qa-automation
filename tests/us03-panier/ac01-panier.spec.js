...
test.describe('US03 - Panier', () => {
  ...
  test('TC-US03-AC01-05 retrait partiel conserve deux produits', async ({
    ...
    await expect
      .poll(() => cartPage.names())
      .toEqual([products.backpack.name, products.boltTShirt.name]);
  });

  test('TC-US03-AC02-02 contenu exact de deux produits', async ({ inventoryPage, cartPage }) => {
    await inventoryPage.add(products.backpack.slug);
    ...
    await expect
      .poll(() => cartPage.names())
      .toEqual([products.backpack.name, products.bikeLight.name]);
  });

  test('TC-US03-AC02-03 contenu du panier conservé pendant la navigation', async ({
    page,
    inventoryPage,
    cartPage,
  }) => {
