...
test.describe('US03 - Panier', () => {
  ...
  test('TC-US03-AC01-01 ajout un produit', async ({
    inventoryPage,
  }) => {
    ...
    await expect(
      inventoryPage.page.locator(
        `[data-test="remove-${products.backpack.slug}"]`,
      ),
    ).toBeVisible();
  });

  test('TC-US03-AC01-02 ajout trois produits', async ({
    inventoryPage,
  }) => {
  ...
  test('TC-US03-AC01-03 retrait depuis catalogue', async ({
    inventoryPage,
  }) => {
    await inventoryPage.add(products.backpack.slug);
  ...
  test('TC-US03-AC01-04 retrait depuis panier', async ({
    inventoryPage,
    cartPage,
  }) => {
    await inventoryPage.add(products.backpack.slug);
  ...
  test(
    'TC-US03-AC01-05 retrait partiel conserve deux produits',
  ...
  test(
    'TC-US03-AC02-02 contenu exact de deux produits',
    async ({ inventoryPage, cartPage }) => {
      await inventoryPage.add(products.backpack.slug);
  ...
  test(
    'TC-US03-AC02-03 contenu du panier conservé pendant la navigation',
  ...
  test(
    'TC-US03-AC02-04 continuer les achats revient au catalogue',
    async ({ page, inventoryPage, cartPage }) => {
      await inventoryPage.openCart();
      await cartPage.continueShopping();
  ...
  test(
    'TC-US03-AC02-05 accès au checkout depuis un panier non vide',
    async ({ page, inventoryPage, cartPage }) => {
      await inventoryPage.add(products.backpack.slug);
      await inventoryPage.openCart();
